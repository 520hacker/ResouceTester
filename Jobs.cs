namespace ResouceTest
{
    ...
    internal class Jobs
    {
        ...
        public static async Task Main(List<DownloadItem> downloadList)
        {
            try
            {
        ...
        private static async Task DownloadAll(IEnumerable<DownloadItem> downloadList)
        {
            foreach (DownloadItem downloadItem in downloadList)
            {
                try
                {
                    Console.WriteLine($"start {downloadItem.Url}");
                    // begin download from url
                    var ds = await DownloadFile(downloadItem).ConfigureAwait(false);
                ...
                {
                    Console.WriteLine($"{downloadItem.Url} + {ex.Message}");
                }
        ...
        private static async Task<DownloadService> DownloadFile(DownloadItem downloadItem)
        {
            ...
            {
                Console.WriteLine($"{downloadItem.FileName} - {ex.Message}");
            }
